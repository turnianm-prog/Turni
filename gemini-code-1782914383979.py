...
if uploaded_excel and uploaded_pdf:
    if st.button("🚀 Elabora e Scarica"):
        with st.spinner("Elaborazione in corso..."):
            ...
            dati = []
            # Scansione di tutte le righe dell'Excel
            for row in range(3, ws_orig.max_row + 1):
                for col_cognome, col_turno in [(1, 2), (5, 6)]:
                    ...
                    if val_cog:
                        ...
                        # Usa il dato del PDF se esiste, altrimenti mantieni quello originale
                        turno_pdf = db.get(m_str)
                        turno_finale = turno_pdf if turno_pdf else str(val_turno_orig or "")
                        
                        dati.append({"c": cog_str, "t": turno_finale})
            
            # Creazione nuovo file Excel
